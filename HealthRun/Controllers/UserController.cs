...
namespace HealthRun.Controllers
{
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            //get  a single user
            var loggIn = await _userManager.GetUserAsync(User);
            List<UserViewModel> result = new List<UserViewModel>();
            //we make sure the currently loggedin user is not shown
            foreach (var user in users.Where(i=>i!=loggIn).OrderBy(i=>i.State != loggIn.State))
            {
